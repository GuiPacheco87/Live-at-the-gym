...
class DataTests(unittest.TestCase):
    def test_snapshot_integrity(self):
        ...
        self.assertTrue(all(g.get('sources') for g in data['gyms']))
        self.assertTrue(any(g['id'].startswith('bluefit/') for g in data['gyms']))
        self.assertTrue(any(g['id'].startswith('smartfit/') for g in data['gyms']))
        self.assertGreater(sum(not any(b in g['name'].lower().replace(' ', '') for b in ('smartfit', 'bluefit')) for g in data['gyms']), 4000)
        self.assertEqual(len(data['profiles']),168)
        self.assertGreaterEqual(len(data['cities']),5570)
        self.assertEqual(len([c for c in data['cities'] if c['state']=='MG']),853)
        self.assertEqual(len({c['state'] for c in data['cities']}),27)
        self.assertEqual(len(data['neighborhoods']),17575)
        self.assertTrue(any(n['city']=='Belo Horizonte' for n in data['neighborhoods']))
